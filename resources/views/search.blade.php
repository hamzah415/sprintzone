...
<div class="max-w-screen-xl mx-auto px-4 md:px-8 py-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <div>
            <h2 class="text-3xl md:text-4xl font-black italic uppercase tracking-tighter">Search <span class="text-orange-500">Result</span></h2>
            <p class="text-gray-500 font-medium"> "{{ $query }}" - {{ $products->count() }} produk ditemukan</p>
        </div>

        {{-- SORT (Bisa di-sort juga) --}}
        <div class="flex items-center gap-2">
            <span class="text-[10px] font-bold text-gray-400 uppercase">Urutkan:</span>
            <div class="flex gap-1">
                <a href="{{ route('search', array_merge(request()->except('sort'), ['sort' => 'latest', 'q' => $query])) }}"
                ...
                <a href="{{ route('search', array_merge(request()->except('sort'), ['sort' => 'price_low', 'q' => $query])) }}"
                ...
                <a href="{{ route('search', array_merge(request()->except('sort'), ['sort' => 'price_high', 'q' => $query])) }}"
    ...
    {{-- PRODUCT GRID (SAMA DENGAN ETALASE) --}}
