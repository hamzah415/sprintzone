...
@section('content')
            ...
            <div class="grid md:grid-cols-2 gap-14">
                    ...
                    <div class="bg-gray-100 rounded-2xl overflow-hidden relative">
                        @php
                            ...
                            if (!$mainImage && $product->variants->isNotEmpty()) {
                                ...
                                if ($variantWithImage) $mainImage = $variantWithImage['image'];
                            }
                        ...
                        @if ($mainImage)
                            <img src="{{ asset('storage/' . $mainImage) }}" alt="{{ $product->name }}" id="mainImage" class="w-full h-auto object-cover">
                        @else
                            <div class="w-full h-[500px] flex items-center justify-center text-gray-400">No Image</div>
                        @endif

                        @if ($totalStock <= 0)
                            <div class="absolute inset-0 bg-black/50 flex items-center justify-center rounded-2xl">
                                <span class="bg-red-600 text-white text-lg font-black px-6 py-2 uppercase italic rounded">Habis</span>
                            </div>
                        @endif
                    </div>
                    ...
                    @if ($allImages->count() > 1)
                        <div class="flex gap-3 overflow-x-auto">
                            @foreach ($allImages as $img)
                                <img src="{{ asset('storage/' . $img) }}" class="w-24 h-24 rounded-xl object-cover cursor-pointer border hover:border-orange-500 transition" onclick="document.getElementById('mainImage').src = this.src">
                            @endforeach
                        </div>
                    @endif
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex items-center gap-3">
                        <span class="uppercase text-xs font-bold tracking-wider text-orange-600">{{ $product->brand->name ?? 'Brand' }}</span>
                    ...
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-1">
                            @for ($i = 1; $i <= 5; $i++)
                                <svg class="w-4 h-4 {{ $i <= 4 ? 'text-orange-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    @php
                        ...
                        if (!$mainPrice && $product->variants->isNotEmpty()) {
                            ...
                            if ($prices->isNotEmpty()) $mainPrice = $prices->min();
                        }
                    ...
                    <div>
                        <div class="flex items-center gap-4">
                            <span id="displayPrice" class="text-4xl font-bold text-gray-900">
                                Rp {{ number_format($hasDiscount ? $mainDiscount : $mainPrice, 0, ',', '.') }}
                            </span>
                            @if ($hasDiscount)
                                <span class="text-lg text-gray-400 line-through">Rp {{ number_format($mainPrice, 0, ',', '.') }}</span>
                            @endif
                        </div>
                    </div>
                    ...
                    {{-- STOCK --}}
                    <div class="flex items-center gap-3">
                        <span id="stockText" class="text-sm font-medium {{ $totalStock > 0 ? 'text-green-600' : 'text-red-500' }}">
                    ...
                    @if ($product->variants->count() > 0)
                        <div>
                            ...
                            <div class="flex flex-wrap gap-3">
                                @foreach ($product->variants->unique('color') as $variant)
                                    <button type="button" class="color-btn border border-gray-300 px-4 py-2 rounded-xl text-sm font-medium transition hover:border-orange-500" data-color="{{ $variant->color }}" data-variant-id="{{ $variant->id }}">
                                        {{ $variant->color }}
                                    </button>
                                @endforeach
                            </div>
                        ...
                        <div id="sizesContainer" class="hidden">
                            <span class="text-sm font-bold text-gray-700">Pilih Ukuran:</span>
                            <div class="flex flex-wrap gap-3 mt-2" id="sizesList">
                                {{-- Diisi via JS --}}
                            </div>
                        </div>
                    ...
                    @auth
                        <div class="flex items-center gap-4 py-5 border-y">
                            ...
                            <div class="flex items-center border rounded-xl overflow-hidden">
                                ...
                                <input type="number" id="qtyInput" value="1" min="1" readonly class="w-14 text-center border-0 outline-none">
                    ...
                    <div>
                        @auth
                            <form action="{{ route('cart.add', $product->id) }}" method="POST" id="cartForm">
                                ...
                                @if ($totalStock > 0)
                                    <button type="submit" class="w-full bg-black hover:bg-orange-600 text-white py-4 rounded-2xl font-semibold transition">+ Add To Cart</button>
                                @else
                                    <button type="button" disabled class="w-full bg-gray-300 text-gray-500 py-4 rounded-2xl font-semibold cursor-not-allowed">Stok Habis</button>
                                @endif
                            </form>
                        @else
                            <a href="{{ route('login') }}" class="block w-full text-center bg-black hover:bg-orange-600 text-white py-4 rounded-2xl font-semibold hover:no-underline">+ Add To Cart</a>
                        @endauth
                    </div>
                    ...
                    {{-- BACK --}}
                    <div>
                        <a href="{{ route('products.etalase') }}" class="text-orange-600 hover:underline text-sm">← Kembali ke Etalase</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    {{-- SCRIPT --}}
    <script>
        const variants = @json($product->variants->toArray());
        
        // Load sizes for each variant
        @foreach($product->variants as $variant)
            variants.find(v => v.id == {{ $variant->id }}).sizes = @json($variant->sizes->toArray());
        @endforeach
        ...
        document.querySelectorAll('.color-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                if (selectedVariant && selectedVariant.sizes && selectedVariant.sizes.length > 0) {
                    ...
                    sizesList.innerHTML = selectedVariant.sizes.map(s => `
                        <button type="button" 
                            class="size-btn border border-gray-200 px-4 py-2 rounded-xl text-sm font-medium transition hover:border-orange-500 ${s.stock <= 0 ? 'opacity-50 cursor-not-allowed' : ''}" 
                            data-size-id="${s.id}" 
                            data-size="${s.size}" 
                            data-stock="${s.stock}"
                            data-price="${s.price || selectedVariant.price}"
                            ${s.stock <= 0 ? 'disabled' : ''}>
                    ...
                    document.querySelectorAll('.size-btn').forEach(sb => {
                        sb.addEventListener('click', function() {
                            ...
                            document.querySelectorAll('.size-btn').forEach(b => b.classList.remove('bg-orange-50', 'border-orange-500'));
                            this.classList.add('bg-orange-50', 'border-orange-500');
                            showVariantInfo(this.dataset);
                        });
                    });
                } else {
        ...
        function showVariantInfo(sizeData) {
            const variant = selectedVariant;
            const sizeStock = parseInt(sizeData.stock);
            const sizePrice = parseInt(sizeData.price) || 0;
            const variantPrice = parseInt(variant.price) || 0;
            const finalPrice = sizePrice > 0 ? sizePrice : variantPrice;

            document.getElementById('variantInfo').classList.remove('hidden');
            document.getElementById('variantSku').innerText = variant.sku || '-';
            document.getElementById('variantPrice').innerText = 'Rp ' + finalPrice.toLocaleString('id-ID');
            document.getElementById('variantStock').innerText = sizeStock;
            document.getElementById('stockText').innerText = sizeStock > 0 ? '✓ Stok tersedia (' + sizeStock + ')' : '✗ Stok habis';
            document.getElementById('variantId').value = variant.id;
            document.getElementById('qtyInput').max = sizeStock;
