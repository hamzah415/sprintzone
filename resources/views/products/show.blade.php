        ...
        <div class="grid md:grid-cols-2 gap-12">
            ...
            <div class="space-y-4">
                ...
                <div class="bg-gray-50 rounded-xl overflow-hidden relative group">
                    @if ($product->image)
                        <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="w-full h-auto object-cover" id="mainImage">
                    @else
                    ...
                    <form action="#" method="POST" class="absolute top-4 right-4">
                        @csrf
                        <button type="button" class="w-10 h-10 bg-white rounded-full shadow-md flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 transition">
                ...
                <div class="flex gap-3 overflow-x-auto">
                    @foreach(json_decode($product->gallery_images) as $img)
                        <img src="{{ asset('storage/' . $img) }}" class="w-20 h-20 object-cover rounded-lg cursor-pointer hover:ring-2 hover:ring-orange-500 transition" onclick="document.getElementById('mainImage').src = this.src">
                    @endforeach
                </div>
                @endif
            </div>
            ...
            <div class="space-y-5">
                ...
                <div class="flex items-center gap-4 py-4 border-y border-gray-100">
                    ...
                    <div class="flex items-center border rounded-lg">
                        <button type="button" onclick="decreaseQty()" class="w-10 h-10 flex items-center justify-center text-gray-600 hover:bg-gray-100">-</button>
                        <input type="number" name="qty" id="qtyInput" value="1" min="1" max="{{ $product->stock }}" class="w-12 text-center border-none outline-none" readonly>
                        <button type="button" onclick="increaseQty()" class="w-10 h-10 flex items-center justify-center text-gray-600 hover:bg-gray-100">+</button>
                    </div>
                </div>
                @endauth
                ...
                <div class="pt-2">
                    @auth
                        <form action="{{ route('cart.add', $product->id) }}" method="POST" class="flex gap-3">
                            ...
                            <button type="submit" class="flex-1 bg-black hover:bg-orange-600 text-white px-8 py-3 rounded-lg font-medium transition">
                                + Add To Cart
                            </button>
                        </form>
                    @else
                        <a href="{{ route('login') }}" class="block w-full text-center px-8 py-3 hover:no-underline bg-black hover:bg-orange-600 text-white rounded-lg font-medium transition">
                            + Add To Cart
                        </a>
                    @endauth
                </div>
