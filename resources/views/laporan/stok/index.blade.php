    ...
    <div class="container-fluid py-4">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
            <div class="bg-blue-500 text-white p-4 rounded-lg">
                <p class="text-xs uppercase">Total Produk</p>
                <p class="text-2xl font-black">{{ $totalProduk }}</p>
            </div>
                ...
                <table class="w-full">
                    ...
                    <tbody>
                        @forelse($variants as $index => $variant)
                            <tr
                                class="border-t {{ $variant->stock == 0 ? 'bg-red-50' : ($variant->stock < 5 ? 'bg-yellow-50' : '') }}">
                                ...
                                <td class="p-4">
                                    @if ($variant->stock == 0)
                                        <span
                                            class="px-2 py-1 text-xs rounded bg-red-100 text-red-600 font-bold">HABIS</span>
                                    @elseif($variant->stock < 5)
                                        <span
                                            class="px-2 py-1 text-xs rounded bg-yellow-100 text-yellow-600 font-bold">LOW</span>
                                    @else
                                        <span
                                            class="px-2 py-1 text-xs rounded bg-green-100 text-green-600 font-bold">READY</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
